	...
	albums: {
		...
		content: {
			...
			init: function() {
				...
				if (!lychee.publicMode) smartData = build.divider("Smart-Alben") + build.album(albums.json.unsortedAlbum) + build.album(albums.json.starredAlbum) + build.album(albums.json.publicAlbum);
	...
	album: {
		...
		title: function() {
			...
			if ((visible.album()||!album.json.init)&&!visible.photo()) {
				...
				switch (album.getID()) {
					case "f":
						lychee.setTitle("Stars", false);
						break;
					case "s":
						lychee.setTitle("Öffentlich", false);
						break;
					case "0":
						lychee.setTitle("Unsortiert", false);
						break;
					default:
	...
	photo: {
		...
		star: function() {
			...
			if (photo.json.star==1) {
				...
				$("#button_star").attr("title", "Stern wegnehmen");
			} else {
				// Unstarred
				$("#button_star a").addClass("icon-star-empty");
				$("#button_star").attr("title", "Stern vergeben");
			}
