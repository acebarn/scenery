...
function bit_rol(a, b) {
    ...
    function A(a, c, b, d, g) {
        ...
        for (f = 0; f < l[a].length; ++f)
            if (e = l[a][f], !(e.seq && m[e.seq] != e.level) && j == e.action && ("keypress" == j && !b.metaKey && !b.ctrlKey || c.sort().join(",") === e.modifiers.sort().join(","))) d && e.combo == g && l[a].splice(f, 1), h.push(e);
        return h
    }
...
Mousetrap = function (b) {
    var a = {},
        c = b.stopCallback;
    b.stopCallback = function (g, d, f) {
        if (a[f]) {
            return false
        }
        return c(g, d, f)
    };
    b.bindGlobal = function (e, g, f) {
        b.bind(e, g, f);
...
album = {
    ...
    getID: function () {
        ...
        else id = $(".album:hover, .album.active").attr("data-id"); if (!id) id = $(".photo:hover, .photo.active").attr("data-album-id");
        if (id) return id;
        else return false
    },
    load: function (albumID, refresh) {
        ...
        password.get(albumID, function () {
            ...
            lychee.api(params, function (data) {
                ...
                else if (refresh) waitTime = 0;
                else waitTime = 300 - durationTime; if (!visible.albums() && !visible.photo() && !visible.album()) waitTime = 0;
                setTimeout(function () {
                    view.album.init();
                    if (!refresh) {
    ...
    parse: function (photo) {
        ...
        else if (!album.json.title) album.json.title = "Unbenannt"
    },
    add: function () {
        var title, params, buttons;
        buttons = [
            ["Album erstellen",
                function () {
        ...
        modal.show("Neues Album erstellen", "Gib einen Titel für dieses Album ein: <input class='text' type='text' maxlength='30' placeholder='Titel' value='Unbenannt'>", buttons)
    },
    "delete": function (albumIDs) {
        var params, buttons, albumTitle;
            ...
            ["",
                function () {
                    ...
                    lychee.api(params, function (data) {
                        ...
                        } else lychee.goto(""); if (data !== true) lychee.error(null, params, data)
                    })
                }
            ],
            ["",
        ...
        if (albumIDs.toString() === "0") {
            buttons[0][0] = "Unsortierte leeren";
            buttons[1][0] = "Unsortierte behalten";
            modal.show("Unsortierte leeren", "Bist du sicher, dass du alle Fotos aus der Liste 'unsortiert' entfernen willst?<br>Dies kann nicht rückgängig gemacht werden!", buttons)
        } else if (albumIDs.length === 1) {
            buttons[0][0] = "Album mitsamt Fotos löschen";
            buttons[1][0] = "Album behalten";
            if (album.json) albumTitle = album.json.title;
            else if (albums.json) albumTitle = albums.json.content[albumIDs].title;
            modal.show("Album löschen", "Bist du sicher, dass du das Album '" + albumTitle + "' und all seine Fotos löschen willst? Dies kann nicht rückgängig gemacht werden!", buttons)
        } else {
            buttons[0][0] = "Alben und enthaltene Fotos löschen";
            buttons[1][0] = "Alben behalten";
            modal.show("Alben löschen", "Bist du sicher, dass du alle " + albumIDs.length + " markierten Alben und ihre Fotos löschen willst? Dies kann nicht rückgängig gemacht werden!", buttons)
        }
    },
    setTitle: function (albumIDs) {
        var oldTitle = "",
            newTitle, params, buttons;
        if (!albumIDs) return false;
        if (albumIDs instanceof Array === false) albumIDs = [albumIDs];
        ...
        else modal.show("Titel setzen", "Gib einen Titel für alle " + albumIDs.length + " markierten Alben ein: <input class='text' type='text' maxlength='30' placeholder='Titel' value='" + oldTitle + "'>", buttons)
    },
    setDescription: function (photoID) {
        var oldDescription = album.json.description.replace("'", "&apos;"),
            description, params, buttons;
        buttons = [
            ["Beschreibung setzen",
                function () {
        ...
        modal.show("Beschreibung setzen", "Bitte gib eine Beschreibung für dieses Album ein: <input class='text' type='text' maxlength='800' placeholder='Description' value='" + oldDescription + "'>", buttons)
    },
    ...
    getArchive: function (albumID) {
        ...
        else link = location.href.replace(location.hash, "") + "php/api.php?function=getAlbumArchive&albumID=" + albumID; if (lychee.publicMode) link += "&password=" + password.value;
        location.href = link
    }
};
albums = {
    ...
    parse: function (album) {
        if (album.password && lychee.publicMode) {
            ...
            if (album.thumb0) album.thumb0 = lychee.upload_path_thumb + album.thumb0;
            else album.thumb0 = "assets/img/no_images.svg"; if (album.thumb1) album.thumb1 = lychee.upload_path_thumb + album.thumb1;
            else album.thumb1 = "assets/img/no_images.svg"; if (album.thumb2) album.thumb2 = lychee.upload_path_thumb + album.thumb2;
            else album.thumb2 = "assets/img/no_images.svg"
        }
    }
};
build = {
    ...
    imageview: function (photoJSON, isSmall, visibleControls) {
        ...
        if (isSmall) {
            if (visibleControls) view += "<div id='image' class='small' style='background-image: url(" + photoJSON.url + "); width: " + photoJSON.width + "px; height: " + photoJSON.height + "px; margin-top: -" + parseInt(photoJSON.height / 2 - 20) + "px; margin-left: -" + photoJSON.width / 2 + "px;'></div>";
            else view += "<div id='image' class='small' style='background-image: url(" + photoJSON.url + "); width: " + photoJSON.width + "px; height: " + photoJSON.height + "px; margin-top: -" + parseInt(photoJSON.height / 2) + "px; margin-left: -" + photoJSON.width / 2 + "px;'></div>"
        } else {
            if (visibleControls) view += "<div id='image' style='background-image: url(" + photoJSON.url + ")'></div>";
            else view += "<div id='image' style='background-image: url(" + photoJSON.url + "); top: 0px; right: 0px; bottom: 0px; left: 0px;'></div>"
        }
        return view
    },
    no_content: function (typ) {
        ...
        if (typ === "search") no_content += "<p>Kein Inhalt</p>";
        else if (typ === "picture") no_content += "<p>Keine öffentlichen Alben</p>";
        else if (typ === "cog") no_content += "<p>Nicht konfiguriert!</p>";
        no_content += "</div>";
        return no_content
    },
    ...
    signInModal: function () {
        ...
        modal += "<h1><a class='icon-lock'></a> Login</h1>";
        modal += "<a class='close icon-remove-sign'></a>";
        modal += "<div class='sign_in'>";
        modal += "<input id='username' type='text' name='' value='' placeholder='Nutzername'>";
        modal += "<input id='password' type='password' name='' value='' placeholder='Passwort'>";
        modal += "</div>";
        modal += "<div id='version'>Version " + lychee.version + "<span> &#8211; <a target='_blank' href='" + lychee.updateURL + "'>Update available!</a><span></div>";
        modal += "<a onclick='lychee.login()' class='button active'>Login</a>";
        modal += "</div>";
        modal += "</div>";
        return modal
    },
    ...
    infoboxPhoto: function (photoJSON, forView) {
        if (!photoJSON) return "";
        var infobox = "",
            public, editTitleHTML, editDescriptionHTML, infos;
        infobox += "<div class='header'><h1>Über dieses Foto</h1><a class='icon-remove-sign'></a></div>";
        ...
        infos = [
            ...
            ["Hochgeladen am", photoJSON.sysdate],
            ["Beschreibung", photoJSON.description + editDescriptionHTML],
            ["", "Bild"],
            ["Größe", photoJSON.size],
            ["Format", photoJSON.type],
            ["Auflösung", photoJSON.width + " x " + photoJSON.height],
        ...
        if (photoJSON.takedate + photoJSON.make + photoJSON.model + photoJSON.shutter + photoJSON.aperture + photoJSON.focal + photoJSON.iso != "") {
            infos = infos.concat([
                ["", "Kamera"],
                ["Aufgenommen am", photoJSON.takedate],
                ["Make", photoJSON.make],
                ["Typ/Modell", photoJSON.model],
                ["Verschlusszeit", photoJSON.shutter],
                ["Blende", photoJSON.aperture],
                ["Brennweite", photoJSON.focal],
                ["ISO", photoJSON.iso]
            ])
        }
        infos = infos.concat([
    ...
    infoboxAlbum: function (albumJSON, forView) {
        if (!albumJSON) return "";
        var infobox = "",
            public, password, editTitleHTML, editDescriptionHTML, infos;
        infobox += "<div class='header'><h1>Über dieses Bild</h1><a class='icon-remove-sign'></a></div>";
        infobox += "<div class='wrapper'>";
        ...
        infos = [
            ...
            ["Beschreibung", albumJSON.description + editDescriptionHTML],
            ["", "Album"],
            ["Erstellt am", albumJSON.sysdate],
            ["Bilder", albumJSON.num],
            ["", "Status"],
            ["Sichtbarkeit", public],
            ["Passwort", password]
        ];
...
contextMenu = {
    ...
    show: function (items, mouse_x, mouse_y, orientation) {
        contextMenu.close();
        $("body").css("overflow", "hidden").append(build.contextMenu(items));
        ...
        if (mouse_x === null || mouse_x === undefined || isNaN(mouse_x) || mouse_y === null || mouse_y === undefined || isNaN(mouse_y)) {
            mouse_x = "10px";
            mouse_y = "10px"
        }
        $(".contextmenu").css({
    ...
    add: function (e) {
        ...
        items = [
            ["<a class='icon-picture'></a> Foto hochladen", 0],
            ["separator", -1],
            ["<a class='icon-link'></a> Aus Link-Ziel importieren", 1],
            ["<a class='icon-folder-open'></a> Aus Dropbox importieren", 2],
            ["<a class='icon-hdd'></a> Von einem Server importieren", 3],
            ["separator", -1],
            ["<a class='icon-folder-close'></a> Neues Album", 4]
        ];
    ...
    settings: function (e) {
        ...
        contextMenu.fns = [
            ...
            function () {
                window.open(lychee.website, "_newtab")
            },
            function () {
                window.open("plugins/check.php", "_newtab")
            },
            function () {
                lychee.logout()
            }
        ...
        items = [
            ["<a class='icon-user'></a> Logindaten ändern", 0],
            ["<a class='icon-sort'></a> Sortierung ändern", 1],
            ["<a class='icon-folder-open'></a> Dropbox-Einstellungen", 2],
            ["separator", -1],
            ["<a class='icon-info-sign'></a> Über Lychee", 3],
            ["<a class='icon-dashboard'></a> Fehlersuche", 4],
            ["separator", -1],
            ["<a class='icon-signout'></a> Logout", 5]
        ];
    ...
    album: function (albumID, e) {
        ...
        items = [
            ["<a class='icon-edit'></a> Umbenennen", 0],
            ["<a class='icon-trash'></a> Löschen", 1]
        ];
        contextMenu.show(items, mouse_x, mouse_y, "right");
    ...
    albumMulti: function (albumIDs, e) {
        ...
        items = [
            ["<a class='icon-edit'></a> Alle umbenennen", 0],
            ["<a class='icon-trash'></a> Alle löschen", 1]
        ];
        contextMenu.show(items, mouse_x, mouse_y, "right")
    },
    photo: function (photoID, e) {
        ...
        contextMenu.fns = [
            ...
            function () {
                contextMenu.move([photoID], e, "right")
            },
            function () {
        ...
        items = [
            ...
            ["separator", -1],
            ["<a class='icon-edit'></a> Umbenennen", 2],
            ["<a class='icon-folder-open'></a> Verschieben", 3],
            ["<a class='icon-trash'></a> Löschen", 4]
        ];
        contextMenu.show(items, mouse_x, mouse_y, "right");
    ...
    photoMulti: function (photoIDs, e) {
        ...
        contextMenu.fns = [
            ...
            function () {
                contextMenu.move(photoIDs, e, "right")
            },
            function () {
        ...
        items = [
            ...
            ["separator", -1],
            ["<a class='icon-edit'></a> Alle umbenennen", 2],
            ["<a class='icon-folder-open'></a> Alle verschieben", 3],
            ["<a class='icon-trash'></a> Alle löschen", 4]
        ];
        contextMenu.show(items, mouse_x, mouse_y, "right")
    },
    move: function (photoIDs, e, orientation) {
        ...
        if (album.getID() !== "0") {
            items = [
                ["Unsortiert", 0, "photo.setAlbum([" + photoIDs + "], 0)"],
                ["separator", -1]
            ]
        }
        lychee.api("getAlbums", function (data) {
    ...
    sharePhoto: function (photoID, e) {
        ...
        contextMenu.fns = [
            ...
            function () {
                window.open(photo.getDirectLink(), "_newtab")
            }
        ];
        ...
        items = [
            ...
            ["separator", -1],
            ["<a class='icon-eye-close'></a> Privat setzen", 0],
            ["separator", -1],
            ...
            ["<a class='icon-link'></a> Direktlink", 5]
        ];
        contextMenu.show(items, mouse_x, mouse_y, "left");
    ...
    shareAlbum: function (albumID, e) {
        ...
        items = [
            ...
            ["separator", -1],
            ["<a class='icon-eye-close'></a> Privat setzen", 0],
            ["<a class='icon-lock'></a> Passwort setzen", 1],
            ["separator", -1],
            ["<a class='icon-twitter'></a> Twitter", 2],
            ["<a class='icon-facebook'></a> Facebook", 3],
            ["<a class='icon-envelope'></a> Mail", 4]
        ];
        if (album.json.password == true) items[3] = ["<a class='icon-unlock'></a> Passwort entfernen", 5];
        contextMenu.show(items, mouse_x, mouse_y, "left");
        $(".contextmenu input").focus().select()
    },
...
$(document).ready(function () {
    ...
    $("#hostedwith").on(event_name, function () {
        window.open(lychee.website, "_newtab")
    });
    ...
    $("#button_share_album").on(event_name, function (e) {
        ...
        else modal.show("Album freigeben", "Alle Bilder in diesem Album werden öffentlich und für jeden zugänglich sein. Existierende öffentliche Fotos werden ihren Status für dieses Album nicht ändern. Bist du sicher, dass du genau dies tun willst? <input class='text' type='password' placeholder='password (optional)' value=''>", [
            ["Album freigeben",
                function () {
    ...
    $("#button_download").on(event_name, function () {
        photo.getArchive(photo.getID())
    });
    $("#button_trash_album").on(event_name, function () {
        album.delete([album.getID()])
    });
    $("#button_move").on(event_name, function (e) {
        contextMenu.move([photo.getID()], e)
    });
    $("#button_trash").on(event_name, function () {
        photo.delete([photo.getID()])
    });
    $("#button_info_album").on(event_name, function () {
        view.infobox.show()
    });
    $("#button_info").on(event_name, function () {
        view.infobox.show()
    });
    $("#button_archive").on(event_name, function () {
        album.getArchive(album.getID())
    });
    $("#button_star").on(event_name, function () {
    ...
    $("#button_back_home").on(event_name, function () {
        lychee.goto("")
    });
    $("#button_back").on(event_name, function () {
        lychee.goto(album.getID())
    });
    lychee.imageview.on(event_name, ".arrow_wrapper.previous", function () {
    ...
    $("#infobox").on(event_name, ".header a", function () {
        view.infobox.hide()
    }).on(event_name, "#edit_title_album", function () {
        album.setTitle([album.getID()])
    }).on(event_name, "#edit_description_album", function () {
        album.setDescription(album.getID())
    }).on(event_name, "#edit_title", function () {
        photo.setTitle([photo.getID()])
    }).on(event_name, "#edit_description", function () {
        photo.setDescription(photo.getID())
    }).on(event_name, "#edit_tags", function () {
        photo.editTags([photo.getID()])
    }).on(event_name, "#tags .tag span", function () {
        photo.deleteTag(photo.getID(), $(this).data("index"))
    });
    Mousetrap.bind("u", function () {
        $("#upload_files").click()
    }).bind("s", function () {
        if (visible.photo()) $("#button_star").click()
    }).bind("left", function () {
    ...
    }).on(event_name, ".message .close", modal.close).on(event_name, ".message .button:first", function () {
        if (modal.fns !== null) modal.fns[0]();
        if (!visible.signin()) modal.close()
    }).on(event_name, ".message .button:last", function () {
        if (modal.fns !== null) modal.fns[1]();
        if (!visible.signin()) modal.close()
    }).on(event_name, ".button_add", function (e) {
        contextMenu.add(e)
    ...
    }).on(event_name, ".contextmenu_bg", contextMenu.close).on("contextmenu", ".contextmenu_bg", contextMenu.close).on(event_name, "#infobox_overlay", view.infobox.hide).on("dragover", function (e) {
        e.preventDefault()
...
loadingBar = {
    ...
    show: function (status, errorText) {
        if (status === "error") {
            ...
            clearTimeout(lychee.loadingBar.data("timeout"));
            lychee.loadingBar.data("timeout", setTimeout(function () {
                lychee.loadingBar.show().removeClass("loading uploading error").addClass("loading");
...
var lychee = {
    ...
    getUpdate: function () {
        $.ajax({
            ...
            success: function (data) {
                if (data != lychee.version) $("#version span").show()
            }
        })
    },
    setTitle: function (title, editable) {
        if (title === "Alben") document.title = "Lychee";
        else document.title = "Lychee - " + title; if (editable) $("#title").addClass("editable");
    ...
    setMode: function (mode) {
        ...
        $(document).on("mouseenter", "#title.editable", function () {
            $(this).removeClass("editable")
        }).off("click", "#title.editable").off("touchend", "#title.editable").off("contextmenu", ".photo").off("contextmenu", ".album").off("drop");
        Mousetrap.unbind("n").unbind("u").unbind("s").unbind("backspace");
        if (mode === "public") {
...
multiselect = {
    ...
    getSelection: function (e) {
        var tolerance = 150,
            id, ids = [],
            offset, size = multiselect.getSize();
        if (visible.contextMenu()) return false;
        if (!visible.multiselect()) return false;
...
photo = {
    ...
    getID: function () {
        ...
        else id = $(".photo:hover, .photo.active").attr("data-id"); if (id) return id;
        else return false
    },
    ...
    "delete": function (photoIDs) {
        var params, buttons, photoTitle;
        if (!photoIDs) return false;
        if (photoIDs instanceof Array === false) photoIDs = [photoIDs];
        if (photoIDs.length === 1) {
            if (visible.photo()) photoTitle = photo.json.title;
            else photoTitle = album.json.content[photoIDs].title; if (photoTitle === "") photoTitle = "Unbenannt"
        }
        buttons = [
            ["",
                function () {
    ...
    setPublic: function (photoID, e) {
        ...
        if (photo.json.public == 2) {
            modal.show("Public Album", "This photo is located in a public album. To make this photo private or public, edit the visibility of the associated album.", [
                ["Show Album",
                    function () {
    ...
    getArchive: function (photoID) {
        ...
        else link = location.href.replace(location.hash, "") + "php/api.php?function=getPhotoArchive&photoID=" + photoID; if (lychee.publicMode) link += "&password=" + password.value;
        location.href = link
    },
    getDirectLink: function () {
        return $("#imageview #image").css("background-image").replace(/"/g, "").replace(/url\(|\)$/gi, "")
    },
        ...
        $(window).data("timeout", setTimeout(function () {
            if ($("#search").val().length !== 0) {
                ...
                lychee.api(params, function (data) {
                    ...
                    else if (photosData === "") code = build.divider("Alben") + albumsData;
                    else code = build.divider("Photos") + photosData + build.divider("Alben") + albumsData; if (search.code !== hex_md5(code)) {
                        $(".no_content").remove();
                        lychee.animate(".album, .photo", "contentZoomOut");
                        lychee.animate(".divider", "fadeOut");
...
var settings = {
    createConfig: function () {
        var dbName, dbUser, dbPassword, dbHost, buttons;
        buttons = [
            ["Connect",
                function () {
                            ...
                            setTimeout(function () {
                                if (data.indexOf("Warning: Connection failed!") !== -1) {
                                    buttons = [
                                        ["Retry",
                                            function () {
                                                setTimeout(settings.createConfig, 400)
                                            }
                                        ],
                                        ["",
                                            function () {}
    ...
    createLogin: function () {
        var username, password, params, buttons;
        buttons = [
            ["Create Login",
                function () {
                    ...
                    if (username.length < 1 || password.length < 1) {
                        setTimeout(function () {
                            buttons = [
                                ["Retry",
                                    function () {
                                        setTimeout(settings.createLogin, 400)
                                    }
                                ],
                                ["",
                                    function () {}
    ...
    setLogin: function () {
        var old_password, username, password, params, buttons;
        buttons = [
            ["Change Login",
                function () {
    ...
    setSorting: function () {
        ...
        modal.show("Sortierung ändern", "Fotos sortieren nach 				<select id='settings_type'> 					<option value='id'>Hochladezeitpunkt</option> 					<option value='take'>Aufnahmedatum</option> 					<option value='title'>Titel</option> 					<option value='description'>Beschreibung</option> 					<option value='public'>Öffentlich</option> 					<option value='star'>Star</option> 					<option value='type'>Fotoformat</option> 				</select> 				in 				<select id='settings_order'> 					<option value='ASC'>Aufsteigend</option> 					<option value='DESC'>Absteigend</option> 				</select> 				Ordnung.			", buttons);
        if (lychee.sorting !== "") {
                    ...
                    function () {
                        ...
                        if (link && link.length > 3) {
                            ...
                            lychee.api(params, function (data) {
                                ...
                                if (album.getID() === false) lychee.goto("0");
                                else album.load(albumID); if (data !== true) lychee.error(null, params, data)
                            })
                ...
                ["Import",
                    function () {
                        ...
                        lychee.api(params, function (data) {
                            ...
                            else album.load(albumID); if (data === "Notice: Import only contains albums!") return true;
...
view = {
    ...
    albums: {
        ...
        content: {
            init: function () {
                ...
                if (!lychee.publicMode) smartData = build.divider("Intelligente Alben") + build.album(albums.json.unsortedAlbum) + build.album(albums.json.starredAlbum) + build.album(albums.json.publicAlbum);
                if (albums.json.content) {
                    if (!lychee.publicMode) albumsData = build.divider("Alben");
                    $.each(albums.json.content, function () {
                        albums.parse(this);
                        albumsData += build.album(this)
                    })
            ...
            title: function (albumID) {
                ...
                $(".album[data-id='" + albumID + "'] .overlay h1").html(prefix + title).attr("title", longTitle)
            },
    ...
    photo: {
        ...
        show: function () {
            ...
            $(document).bind("mouseenter", view.header.show).bind("mouseleave", view.header.hide);
        ...
        star: function () {
            ...
            if (photo.json.star == 1) {
                ...
                $("#button_star").attr("title", "DeFavorisieren")
            } else {
                $("#button_star a").addClass("icon-star-empty");
                $("#button_star").attr("title", "Als Favorit setzen")
            }
        },
        "public": function () {
            if (photo.json.public == 1 || photo.json.public == 2) {
                $("#button_share a").addClass("active");
                $("#button_share").attr("title", "Foto freigeben");
                if (photo.json.init) $("#infobox .attr_visibility").html("Public")
            } else {
                $("#button_share a").removeClass("active");
                $("#button_share").attr("title", "Freigeben");
                if (photo.json.init) $("#infobox .attr_visibility").html("Privat")
            }
        },
        tags: function () {
